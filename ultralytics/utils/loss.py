...
# 全局 IoU 类型覆盖 — 由 train.py 在训练前设置
IOU_TYPE_OVERRIDE = None
...
class BboxLoss(nn.Module):
    ...
    def __init__(self, reg_max: int = 16, iou_type: str = "CIoU"):
        ...
        Args:
            ...
            iou_type (str): IoU loss type. One of 'IoU', 'GIoU', 'DIoU', 'CIoU', 'SIoU'. Default: 'CIoU'.
        """
        super().__init__()
        self.dfl_loss = DFLoss(reg_max) if reg_max > 1 else None
        self.iou_type = iou_type
        # Build kwargs dict for bbox_iou
        self.iou_kwargs = {self.iou_type: True} if self.iou_type != "IoU" else {}
    ...
    def forward(
        ...
        iou = bbox_iou(pred_bboxes[fg_mask], target_bboxes[fg_mask], xywh=False, **self.iou_kwargs)
...
class v8DetectionLoss:
    ...
    def __init__(self, model, tal_topk: int = 10, tal_topk2: int | None = None, iou_type: str | None = None):
        """Initialize v8DetectionLoss with model parameters and task-aligned assignment settings.

        Args:
            model: The YOLO model (de-paralleled).
            tal_topk (int): Top-k for task-aligned assigner.
            tal_topk2 (int | None): Second stage top-k.
            iou_type (str | None): IoU type for box loss. If None, reads from model.args or defaults to 'CIoU'.
        """
        ...
        # Determine IoU type: global override > explicit arg > model attribute > default "CIoU"
        global IOU_TYPE_OVERRIDE
        if IOU_TYPE_OVERRIDE is not None:
            self.iou_type = IOU_TYPE_OVERRIDE
        elif iou_type is not None:
            self.iou_type = iou_type
        elif hasattr(model, "iou_type"):
            self.iou_type = model.iou_type
        else:
            self.iou_type = "CIoU"
