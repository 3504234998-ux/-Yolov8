...
class SimSPPF(nn.Module):
    ...
    def __init__(self, c1: int, c2: int, k: int = 5, n: int = 3, shortcut: bool = False):
        """Initialize SimSPPF with input/output channels, kernel size, and number of pooling layers.

        Args:
            c1 (int): Input channels.
            c2 (int): Output channels.
            k (int): Kernel size of the pooling layers.
            n (int): Number of pooling iterations (default 3, equivalent to SPP(k=(5,9,13))).
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Apply sequential Conv+ReLU operations to input and return concatenated feature maps."""
        y = [self.cv1(x)]
        y.extend(self.m[i](y[-1]) for i in range(getattr(self, "n", 3)))
        y = self.cv2(torch.cat(y, 1))
        return y + x if getattr(self, "add", False) else y
...
class SPPCSPC(nn.Module):
    ...
    def __init__(self, c1: int, c2: int, k: tuple[int, ...] = (5, 9, 13), e: float = 0.5):
        """Initialize SPPCSPC with input/output channels and pooling kernel sizes.

        Args:
            c1 (int): Input channels.
            c2 (int): Output channels.
            k (tuple): Kernel sizes for max pooling operations.
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Apply SPPCSPC forward pass with CSP-based multi-scale pooling."""
...
class DWConv_Bottleneck(nn.Module):
    ...
    def __init__(
        ...
        """
        super().__init__()
        self.cv1 = DWConv(c1, c2, k[0], 1)
        self.cv2 = DWConv(c2, c2, k[1], 1)
        self.add = shortcut and c1 == c2

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Apply DWConv bottleneck operations."""
        return x + self.cv2(self.cv1(x)) if self.add else self.cv2(self.cv1(x))


class C2f_DWConv(nn.Module):
    """C2f with full Depthwise Separable Convolutions (cv1/cv2 + Bottlenecks all DWConv)."""

    def __init__(self, c1: int, c2: int, n: int = 1, shortcut: bool = False, g: int = 1, e: float = 0.5):
        super().__init__()
        self.c = int(c2 * e)
        self.cv1 = DWConv(c1, 2 * self.c, 1, 1)
        self.cv2 = DWConv((2 + n) * self.c, c2, 1, 1)
        self.m = nn.ModuleList(
...
class C2f_Light(nn.Module):
    """C2f 轻量化变体: 1x1 Conv 保持标准卷积(权重兼容预训练), 仅 Bottleneck 中的 3x3 Conv 替换为 DWConv.

    与 C2f_DWConv 的区别:
    - C2f_DWConv: cv1/cv2 + Bottleneck 全改为 DWConv → 预训练权重无法迁移
    - C2f_Light:   cv1/cv2 保持标准 Conv，仅 Bottleneck 内用 DWConv → 预训练权重部分兼容
